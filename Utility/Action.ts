...
export async function waitforElementPresent(webElement, timeout = 10000) {
  try {
    await browser.waitUntil(EC.visibilityOf(webElement), {
      timeout: timeout,
      timeoutMsg: "Failed, after waiting for the element to be visible",
    });
...
export async function waitUntilClickable(webElement, timeout: number) {
  try {
    let result = await browser
      .waitUntil(EC.elementToBeClickable(webElement), {
        timeout: timeout,
